...
function create_magic ($maxup, $maxdown, $p2plow,$maskq) {
  ...
  /* Create new queues */
  $config['shaper']['queue'][$queuei]['descr'] = "a_web下载";
  $config['shaper']['queue'][$queuei]['targetpipe'] = 1;
  $config['shaper']['queue'][$queuei]['weight'] = 50;
  $config['shaper']['queue'][$queuei]['mask'] = destination;
  $queuei++;
  $config['shaper']['queue'][$queuei]['descr'] = "a_p2p下载";
  $config['shaper']['queue'][$queuei]['targetpipe'] = 1;
  $config['shaper']['queue'][$queuei]['weight'] = 15;
  $config['shaper']['queue'][$queuei]['mask'] = destination;
  $queuei++;
  $config['shaper']['queue'][$queuei]['descr'] = "a_udp下载";
  $config['shaper']['queue'][$queuei]['targetpipe'] = 1;
  $config['shaper']['queue'][$queuei]['weight'] = 15;
  $config['shaper']['queue'][$queuei]['mask'] = destination;
  $queuei++;
  $config['shaper']['queue'][$queuei]['descr'] = "a_其他下载";
  $config['shaper']['queue'][$queuei]['targetpipe'] = 1;
  ...
  $config['shaper']['rule'][$rulei]['descr'] = "a_web下载";
  ...
  $rulei++; 
  $config['shaper']['rule'][$rulei]['descr'] = "a_ftp下载";
  $config['shaper']['rule'][$rulei]['targetqueue'] = 0;
  $config['shaper']['rule'][$rulei]['interface'] = "wan";
  $config['shaper']['rule'][$rulei]['direction'] = "in";
  $config['shaper']['rule'][$rulei]['source']['any'] = TRUE;
  ...
  $config['shaper']['rule'][$rulei]['protocol'] = "tcp";
  $rulei++;
  $config['shaper']['rule'][$rulei]['descr'] = "a_p2p下载";
  ...
  $config['shaper']['rule'][$rulei]['protocol'] = "udp";
  $rulei++;
  $config['shaper']['rule'][$rulei]['descr'] = "a_udp下载";
  ...
  $config['shaper']['rule'][$rulei]['protocol'] = "udp";
  $rulei++;
  $config['shaper']['rule'][$rulei]['descr'] = "a_其他下载";
  ...
  $config['shaper']['rule'][$rulei]['destination']['network'] = lan;
  $rulei++;
  $config['shaper']['rule'][$rulei]['descr'] = "a_上传";
...
if ($_POST) {
    ...
    if ($_POST['install']) {
        ...
        if (($_POST['maxup'] && !is_numericint($_POST['maxup']))) {
            $input_errors[] = "总上行速度必须是一个整数。";
        }
        if (($_POST['maxdown'] && !is_numericint($_POST['maxdown']))) {
            $input_errors[] = "总下行速度必须是一个整数。";
        }
        if (!$input_errors) {
          if ($_POST['install']) {
	     	 unset ($config['shaper']);
  ...
  <tr> 
    <td class="tabcont">
     <table width="100%" border="0" cellpadding="6" cellspacing="0">
        <tr valign="top">
          <td width="22%" class="vncellreq">总下行<br>
            速度 </td>
        ...
        <tr valign="top">
          <td width="22%" class="vncellreq">总上行<br>
            速度</td>
		...
		</strong>在您填好最大上行和下行速度参数，按下“安装／更新”按钮后，集体限速会为您创建最佳的规则／管道／队列。
		这些设置可以尽力的发挥出所有带宽，但稳定性不如单IP限速。
        <br>
		<span class="red"><strong>此设置与<a href="firewall_shaper_easy.php">简单设置</a>以及<a href="firewall_shaper_magic.php">流控精灵</a>不能同时使用。 </strong></span></span>
	</td>
    </tr>
</table>
</form>
